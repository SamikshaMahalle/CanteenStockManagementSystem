...
public class DeleteDistributor extends JFrame implements ActionListener
{
	JFrame jf;
	...
	DeleteDistributor()
	{
	    ...
	    l6=new JLabel(" Delete Distributor ");
 ...
 l1.setBounds(150,120,130,25);
		...
		t1.setBounds(320,120,100,25);t1.setToolTipText("Enter distributor id to delete Distributor");
		jf.add(t1);
  ...
  l2.setBounds(150,160,170,25);
		...
		t2.setBounds(320,160,200,25);t2.setToolTipText("Enter distributor name to delete Distributor");
		jf.add(t2);
   ...
   l5.setBounds(150,280,170,25);
		...
		t5.setBounds(320,280,200,25);jf.add(t5);
		 ...
		 b0.setBounds(150,330,100,35);b0.setToolTipText("click to open distributor details");
	     jf.add(b0); b0.addActionListener(this);

	     b1 = new JButton("Delete",new ImageIcon("images//delete.png"));
		 b1.setBounds(300,330,120,35);b1.setToolTipText("click to delete distributor details");
		...
		b3.setBounds(600,330,100,35);b3.setToolTipText("click to view all distributor details");
      ...
      model.addColumn("D_EMAILID");
	     ...
	     jf.setTitle("Delete Distributor");
	     jf.setSize(900,700);
	...
	if(ae.getSource()==b0)
      {
	     ...
	     {
		  JOptionPane.showMessageDialog(this,"Please enter distributor id or name !","Warning!!!",JOptionPane.WARNING_MESSAGE);
	      }
	      else
	      {//fetch
	      try
		...
		System.out.println("Connected to database.");
         ps=con.prepareStatement("select * from distributor where did='"+t1.getText()+"' or dname='"+t2.getText()+"'");
	      rs=ps.executeQuery();
		  while(rs.next())
	      {
	 ...
	 {//delete
	 	if(((t1.getText()).equals(""))&&((t2.getText()).equals("")))
	     {
		  JOptionPane.showMessageDialog(this,"Please enter distributor id or name !","Warning!!!",JOptionPane.ERROR_MESSAGE);
	      }
	      else
	      {
	      try
		...
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/canteen_stock","root","root");
		System.out.println("Connected to database.");
	    ps=con.prepareStatement("delete from distributor where did='"+t1.getText()+"' or dname='"+t2.getText()+"'");
	    ps.executeUpdate();
